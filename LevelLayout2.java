...
import gameData.Asteroid;
import gameData.Bullet;
import gameData.CollisionRect;
import gameData.Explosion;
import gameData.Level2;
import gameData.Virus;
import gameData.VirusLevel2;
...
public class LevelLayout2 extends ScreenAdapter{
	...
	LevelLayout2(final VirusFighter game){
		...
		image = new Texture("Background4.png");
		...
		MediumGreen = new BitmapFont (Gdx.files.internal("myfont6greenMedium.fnt") );
		congratesGreen = new BitmapFont (Gdx.files.internal("myfont6green.fnt") );
		scoreFont = new BitmapFont (Gdx.files.internal("myfont.fnt") );
		scoreFont2 = new BitmapFont (Gdx.files.internal("myfont2.fnt") );
		scoreWordFont = new BitmapFont (Gdx.files.internal("myfont2.fnt") );
		NumOfviruses = new BitmapFont (Gdx.files.internal("myfont2.fnt") );
        ...
        gameOver = new BitmapFont (Gdx.files.internal("myfont3.fnt"));
        gameOverRed = new BitmapFont (Gdx.files.internal("myfont4Red.fnt"));
		...
		asteroid = new Texture(v.getAsteroid());
		virus = new Texture(v.getVirus2());
	...
	public void render (float delta) {
    	...
    	// update explosion
    	ArrayList<Explosion>explosionRemove =new ArrayList<Explosion>();
    	...
    	for(Bullet bullet : bulletManager) {  // for each bullet in bullets list
    		...
    		for(VirusLevel2 virus :virusManager) {  //for each virus in the array list
    			if (bullet.getCollisionRect().collidesWith(virus.getCollisionRect())) {  //collision happend
    				virusRemove.add(virus);
    				bulletRemove.add(bullet);
    				explosions.add(new Explosion(virus.getx(),virus.gety(),30));
    				scoreCounter+=1;
    				score +=100;
    			}
    		}
		...
		for(VirusLevel2 virus : virusManager) {
			if(virus.getCollisionRect().collidesWith(playerRect)) {
				...
				health-=0.1;
				if(health<=0) {
					explosions.add(new Explosion(virus.getx(),virus.gety(),70));
				}
			}
		}
		...
		for(Asteroid asteroid : asteroidManager) {
			if(asteroid.getCollisionRect().collidesWith(playerRect)) {
				...
				health-=0.3;
				if(health<=0) {
					explosions.add(new Explosion(spaceShipLoc.x,spaceShipLoc.y,70));
				}
			}
		}
